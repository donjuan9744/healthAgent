...
async function generateWeeklyPlan({ savedWorkouts, goals, preferences }) {
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a workout planning coach. Return strict JSON only that matches the schema exactly, with no markdown or extra text. Build exactly 7 days. For each exercise include name, type ('reps' or 'time'), sets, reps, and seconds. Use null for reps when type is 'time', and null for seconds when type is 'reps'.",
      },
      {
        role: "user",
...
app.post("/generate-weekly-plan", async (req, res, next) => {
  try {
    const { userId, savedWorkouts, goals, preferences } = req.body || {};
    ...
    const generatedPlan = await generateWeeklyPlan({ savedWorkouts, goals, preferences });
    ...
    await db.collection("users").doc(userId).collection("plans").doc(currentWeek).set(planToStore);
...
app.get("/weekly-plan/:userId", async (req, res, next) => {
  try {
    ...
    const planDoc = await db.collection("users").doc(userId).collection("plans").doc(currentWeek).get();
